...
//// Here we do some shenanigans to be able to outsource the `UnmegedLeaves` structure
#[cfg(not(feature = "sumac"))]
pub mod unmerged_leaves{
    use serde::{Deserialize, Serialize};
    use tls_codec::{TlsSize, TlsSerialize};

    use crate::prelude::LeafNodeIndex;
    use thiserror::*;
...
mod unmerged_leaves{
    ...
    impl UnmergedLeaves {
        ...
        pub(crate) fn add(&mut self, leaf_index: LeafNodeIndex) {
            // The list of unmerged leaves must be sorted. This is enforced upon
            // deserialization. We can therefore safely insert the new leaf at the
            // correct position.
            let position = self.list.binary_search(&leaf_index).unwrap_or_else(|e| e);
        ...
        pub(crate) fn list(&self) -> &[LeafNodeIndex] {
            self.list.as_slice()
        }

        /// Set the list of unmerged leaves.
        pub(crate) fn set_list(&mut self, list: Vec<LeafNodeIndex>) {
            self.list = list;
        }
    }

...
use crate::{
    ...
    error::LibraryError,
    prelude::{Credential, CredentialType, LeafNodeIndex, PathSecret, Secret},
    tree_sumac::nodes::{
        encryption_keys::{AeadCiphertext, SymmetricKey},
        traits::{Leaf, White},
        PlainUpdatePathNode,
    },
...
impl ConcreteNode for LeafNodeTMKA {
    ...
    fn encrypt_path_secret(
        ...
        key.encrypt(crypto, plain.path_secret().clone().secret().as_slice())
    }
...
impl LeafNodeTMKA {
    pub(crate) fn derive_regeneration_secret(
        &self,
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
        ...
        self.leaf_secret()
            .clone()
            .derive_secret(crypto, ciphersuite, "regen")
    }

    pub fn derive_whole_leaf_regeneration(
        &self,
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
    ...
    pub fn absorb_regeneration_secret(
        &self,
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
        regeneration_secret: Secret,
    ) -> Result<Self, CryptoError> {
        let input = [
            self.leaf_secret().clone().as_slice(),
            regeneration_secret.as_slice(),
        ]
        .concat();

        let new_leaf_secret = crypto.hkdf_expand(
            ciphersuite.hash_algorithm(),
            input.as_slice(),
            &[],
            ciphersuite.hash_length(),
        )?;
        let encryption_key = SymmetricKey::derive_from_path_secret(
            crypto,
            ciphersuite,
            &PathSecret::from(Secret::from_slice(new_leaf_secret.as_slice())),
        )
        ...
        match &self.node {
            Some(content) => write!(
                ...
                content
                    .path_secret
                    .clone()
                    .map_or(0, |secret| secret.path_secret.as_slice()[0])
            ),
            None => write!(f, "xx"),
        }
    }
...
impl From<ParentNodeTMKA> for OptionParentNodeTMKA {
    fn from(value: ParentNodeTMKA) -> Self {
        OptionParentNodeTMKA { node: Some(value) }
    }
}
...
impl ParentNodeTMKA {
    ...
    pub fn new_from_path_secret(
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
        path_secret: PathSecret,
        unmerged_leaves: Option<UnmergedLeaves>,
    ) -> Result<Self, LibraryError> {
        let encryption_key =
            SymmetricKey::derive_from_path_secret(crypto, ciphersuite, &path_secret)?;
...
impl ParentNodeTMKA {
    ...
    pub fn derive_whole_parent_regeneration(
        &self,
        crypto: &impl OpenMlsCrypto,
        ciphersuite: Ciphersuite,
    ) -> Result<Self, CryptoError> {
        let secret = self.path_secret.clone().unwrap().secret().derive_secret(
            crypto,
            ciphersuite,
            "regen",
        )?;
    ...
    pub fn absorb_regeneration_secret(
        ...
        let input = [
            self.path_secret.clone().unwrap().secret().as_slice(),
            regeneration_secret.as_slice(),
        ]
        .concat();

        let new_path_secret = crypto.hkdf_expand(
            ciphersuite.hash_algorithm(),
            input.as_slice(),
            &[],
            ciphersuite.hash_length(),
        ...
        let encryption_key =
            SymmetricKey::derive_from_path_secret(crypto, ciphersuite, &new_path_secret)
                .expect("Derivation failed");

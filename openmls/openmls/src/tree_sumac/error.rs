//! TreeSumac Error errors
//!
//! This module exposes [`ApplyUpdatePathError`] and [`PublicTreeError`].
// THis module is a copy of the one of the TreeSumac
// TODO: clean some imports that comes from the prelude !
...
use super::*;
use crate::{
    binary_tree::{MlsBinaryTreeDiffError, MlsBinaryTreeError}, ciphersuite::signable::SignatureError, error::LibraryError, prelude::{PathSecretError, RatchetTreeError},
};
...
pub enum TreeSumacError {
    ...
    NoPrivateKeyFound,
    /// See [`TreeSumacSetPathError`] for more details.
    #[error(transparent)]
    SetPathError(#[from] DerivePathError),
    /// See [`MlsBinaryTreeError`] for more details.
    #[error(transparent)]
    BinaryTreeError(#[from] MlsBinaryTreeError),
    /// See [`TreeSumacDiffError`] for more details.
    #[error(transparent)]
    TreeSumacDiffError(#[from] TreeSumacDiffError),
...
/// TreeSumac set path error
#[derive(Error, Debug, PartialEq, Clone)]
pub enum TreeSumacAddLeaf {
    /// See [`LibraryError`] for more details.
    #[error(transparent)]
    LibraryError(#[from] LibraryError),
...
/// TreeSumac from nodes error
#[derive(Error, Debug, PartialEq, Clone)]
pub enum TreeSumacFromNodesError {
    /// See [`LibraryError`] for more details.
    #[error(transparent)]
    LibraryError(#[from] LibraryError),
...
/// TreeSumac parent hash error
#[derive(Error, Debug, PartialEq, Clone)]
pub(crate) enum TreeSumacParentHashError {
    /// See [`LibraryError`] for more details.
    #[error(transparent)]
    LibraryError(#[from] LibraryError),
...
/// TreeSumac parent hash error
#[derive(Error, Debug, PartialEq, Clone)]
pub(crate) enum TreeSumacDiffError {
    /// See [`LibraryError`] for more details.
    #[error(transparent)]
    LibraryError(#[from] LibraryError),
    #[error(
...
pub(crate) enum TreeKemError {
    ...
    #[error(transparent)]
    LibraryError(#[from] LibraryError),
    /// See [`TreeSumacError`] for more details.
    #[error(transparent)]
    TreeSumacError(#[from] TreeSumacError),
    /// See [`TreeSumacDiffError`] for more details.
    #[error(transparent)]
    TreeSumacDiffError(#[from] TreeSumacDiffError),
    /// See [`PathSecretError`] for more details.

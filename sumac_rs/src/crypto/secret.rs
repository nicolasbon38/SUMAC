...
use openmls::prelude::{Ciphersuite, CryptoError, OpenMlsCrypto, OpenMlsRand};
use tls_codec::SecretVLBytes;

use crate::errors::SumacError;

use openmls::ciphersuite::Secret as OpenmlsSecret;
...
impl Into<OpenmlsSecret> for Secret{
    fn into(self) -> OpenmlsSecret {
        OpenmlsSecret::from_slice(self.as_slice())
    }
}

impl From<OpenmlsSecret> for Secret{
    fn from(secret : OpenmlsSecret) -> Secret {
        Self::from_slice(secret.as_slice())
    }
}

impl Default for Secret {
    fn default() -> Self {
        OpenmlsSecret::default().into()
    }
}
...
impl Secret {
    ...
    pub(crate) fn random(
        ...
        Ok(Secret {
            value: rand
                ...
                .map_err(|_| SumacError::CryptoError(CryptoError::InsufficientRandomness))?
                .into(),
        })
    }
    ...
    pub(crate) fn derive_secret(
        ...
        crypto.hkdf_expand(ciphersuite.hash_algorithm(), self.as_slice(), &[], ciphersuite.hash_length()).map(|vl_bytes| Self::from_slice(vl_bytes.as_slice())).map_err(|err| SumacError::CryptoError(err))
    }

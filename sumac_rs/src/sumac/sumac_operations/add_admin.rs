...
use openmls::{
    prelude::{Ciphersuite, Credential, HpkeCiphertext},
    tree_sumac::{
...
pub fn add_admin_committer(
    ...
    SumacError,
> {
    let new_admin = all_admins
        .get(username_new_admin)
        .expect(&format!("{} not found", username_new_admin));
    ...
    group_committer.sumac_group_key = group_committer.cgka().group_key.clone();
    ...
    let encrypted_regeneration_tree = derived_tree.encrypt_hpke(
        provider,
        ciphersuite,
        new_admin.encryption_keypair()?.public_key(),
    );

    // encrypts the new group key

    //retrieves the key of its tree
    let key_tree = &group_committer.tmka.group_key;
    let encrypted_group_key = key_tree
        .encrypt(
            provider.crypto(),
            ciphersuite,
            group_committer.sumac_group_key.clone().as_slice(),
        )
...
pub fn add_admin_new_admin(
    ...
    let res = all_admin_groups
        .insert(username_new_admin.to_string(), new_admin_group)
        .is_none();
    assert!(res);
...
pub fn add_admin_standard_users(
    ...
    for (_, user_group) in all_users_group.iter_mut() {
        ...
        let res = user_group
            .forest_mut()
            .insert(username_new_admin.to_string(), new_group)
            .is_none();
        assert!(res);
...
fn test_add_admin() {
    ...
    for (admin_name, _) in all_admins.iter() {
        ...
        check_sync_sumac(&all_admins_groups, &all_users_groups);
        committer_name = all_admins_groups
            .keys()
            .choose(&mut rng)
            .unwrap()
            .to_string();
...
pub fn add_admin_only_one_other_admin(
    ...
    let group = all_admin_groups.get_mut(username_target_admin).unwrap();
...
pub fn add_admin_only_one_standard_user(
    ...
    let regeneration_set =
        tree_committer.build_regeneration_path(provider, ciphersuite, &own_leaf_node_index, true);

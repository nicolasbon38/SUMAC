...
fn filling_sumac_group_memory_optimized(n_admins: usize, n_users: usize) -> SumacState {
    ...
    let mut index_admin_3 = rng.gen_range(0..n_users);
    while (index_admin_1 == index_admin_3) || (index_admin_2 == index_admin_3) {
        index_admin_3 = rng.gen_range(0..n_users);
    }
    ...
    let (mut all_admin_groups, mut all_user_groups) =
        create_large_sumac_group_memory_optimized_for_benchmarks(
            ...
            &all_admins,
            &all_users,
            &vec![index_admin_1, index_admin_2],
            &vec![index_user_1, index_user_2],
        )
        .unwrap();
            ...
            |b, &_n| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let (_, commit_unicast, encrypted_regeneration_set, leaf_index_new_user) =
                            add_user_sumac_committer(
                                &mut local_state,
                                &provider,
                                ciphersuite,
            ...
            |b, &_n| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let _ = add_admin_sumac_committer(
                            &mut local_state,
                            &provider,
                            ciphersuite,
                            n_admins,
            ...
            |b, &_n| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let (_cb, commit_unicast, _ers, leaf_index) = update_user_sumac_committer(
                            &mut local_state,
                            &provider,
                            ciphersuite,
                            &username_committer,
...
) -> (CommitTMKABroadcast, EncryptedRegenerationSet, LeafNodeIndex) {
            ...
            |b, &_n| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let (_cb, encrypted_regeneration_set, leaf_index) =
            ...
            |b, &_n| {
                b.iter_custom(|iters| {
                    ...
                    for _ in 0..iters {
                        ...
                        let (commit_broadcast, _ers, leaf_index) = remove_user_sumac_committer(
                            &mut local_state,
                            &provider,
                            ciphersuite,
                            &username_committer,
